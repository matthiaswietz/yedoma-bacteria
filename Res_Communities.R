...
amp_heatmap(amp_subset_samples(
  ...
  order_x_by=c(
    ...
    "fSW+YE_s","SW+YE_w","fSW+YE_w"),
  tax_show = c(
    "SAR11 clade","Pseudomonadales","Flavobacteriales",
    "Enterobacterales","Rhodobacterales","Pirellulales",
    "Burkholderiales","Microtrichales","Micrococcales",
    "Thermomicrobiales","KD4-96_uc","Gitt-GS-136_uc",
    "Gaiellales","Rhizobiales","Solirubrobacterales"),
  order_y_by=c(
...
amp_heatmap(amp_subset_samples(
  ...
  tax_aggregate = "Genus",
  tax_show = c(
    "Psychrobacter","Colwellia","Polaribacter",
    "Moritella","Flavobacterium","Maribacter",
    "Saccharospirillaceae_uc","Marmoricola",
    "SAR11_Clade Ia","NS9 marine group_uc",
    "Marinimicrobia (SAR406 clade)_uc","Gaiella",
    "Methyloceanibacter","Nocardioides",
    "Alphaproteobacteria_uc","Oryzihumus",
    "Gitt-GS-136_uc","Aurantivirga","Pseudohongiella",
    "Methyloligellaceae_uc","Magnetospiraceae_uc",
    "Solirubrobacter","Iamia","Gemmatimonadaceae_uc",
    "Flavobacteriaceae_uc","67-14_uc","MB-A2-108_uc",
    "Vicinamibacterales_uc","KD4-96_uc"),
  order_y_by = c(
...
# Assign origin YE/SW
oriGen %>% filter(Genus %in% c(
  "Psychrobacter","Colwellia","Polaribacter",
  "Moritella","Flavobacterium","Maribacter",
  "Saccharospirillaceae_uc","Marmoricola",
  "KD4-96_uc","SAR11_Clade Ia","Gitt-GS-136_uc",
  "NS9 marine group_uc","Magnetospiraceae_uc",
  "Marinimicrobia (SAR406 clade)_uc","Oryzihumus",
   "Methyloceanibacter","Nocardioides",
  "Aurantivirga","Pseudohongiella","Gaiella",
  "Methyloligellaceae_uc","Alphaproteobacteria_uc",
  "Solirubrobacter","Iamia","Gemmatimonadaceae_uc",
  "Flavobacteriaceae_uc","67-14_uc","MB-A2-108_uc",
  "Vicinamibacterales_uc")) %>%
...
AlphaDiv %>% filter(!type %in% c(
  "fSW+YE_w_mon","SW+YE_w_mon","fSW+YE_s_mon","SW+YE_s_mon") &
    !days %in% c("4","12","29","43","71")) %>%
  reshape2::melt() %>%
  filter(variable %in% c("invSimpson","richness")) %>%
ggplot() +
